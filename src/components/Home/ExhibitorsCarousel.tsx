...
export default function ExhibitorsCarousel({ heading, ctaname, ctalink }: any) {
  const [pageData, setPageData] = useState<any | null>(null);
  useEffect(() => {
    fetch("/api/exhibitors-2025", { cache: "no-store" })
      .then((res) => {
  ...
  return (
    <div className="">
      <div className="container mx-auto px-4">
        ...
        >
          {pageData.slice(0, 20).map((item: any) => (
            <SwiperSlide key={item.id}>
              <div className="relative overflow-hidden group p-4 border border-primary rounded-lg bg-white text-center  flex justify-center flex-col items-center h-50">
                <img
                  src={item.company_logo}
                  alt={item.company_name?.value}
